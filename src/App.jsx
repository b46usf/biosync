...
const ActivityTrendChart = lazy(() =>
  import("./ChartViews.jsx").then((module) => ({
    default: module.ActivityTrendChart,
  })),
);
const HealthSparkline = lazy(() =>
  import("./ChartViews.jsx").then((module) => ({
    default: module.HealthSparkline,
  })),
);
...
const trendData = [
  { day: "Sen", steps: 5200, heart: 77 },
  { day: "Sel", steps: 7900, heart: 74 },
  { day: "Rab", steps: 6100, heart: 76 },
  { day: "Kam", steps: 10200, heart: 72 },
  { day: "Jum", steps: 8400, heart: 75 },
  { day: "Sab", steps: 11200, heart: 71 },
...
const challenges = [
  {
    title: "Move 20 km this week",
    sub: "Weekly movement challenge",
    progress: 62,
    stat: "12.4 / 20 km",
    people: "2,430 joined",
    tone: "lime",
...
const createDefaultActivities = () =>
  initialActivities.map((item, index) => ({ ...item, id: `sample-${index}` }));

function serializeDemoData(data) {
...
function summarizeActivities(activities) {
  ...
  const totalSeconds = activities.reduce(
    (sum, activity) => sum + parseDuration(activity.time),
    0,
  );
  const totalDistance = activities.reduce(
    (sum, activity) => sum + parseKilometers(activity.distance),
    0,
  );
  const paceActivities = activities.filter((activity) =>
    ["Run", "Walk"].includes(activity.type),
  );
  const paceSeconds = paceActivities.reduce(
    (sum, activity) => sum + parseDuration(activity.time),
    0,
  );
  const paceDistance = paceActivities.reduce(
    (sum, activity) => sum + parseKilometers(activity.distance),
    0,
  );
  const hours = Math.floor(totalSeconds / 3600);
  const minutes = Math.floor((totalSeconds % 3600) / 60);
  const secondsPerKm = paceDistance
    ? Math.round(paceSeconds / paceDistance)
    : 0;
  return {
    distance: new Intl.NumberFormat("id-ID", {
      maximumFractionDigits: 1,
    }).format(totalDistance),
    activeTime: hours ? `${hours}j ${minutes}m` : `${minutes}m`,
...
function showModal(options) {
  return Swal.fire({
    ...
    customClass: { ...modalClasses, ...options.customClass },
  });
}
...
function App() {
  ...
  const [activityList, setActivityList] = useState(createDefaultActivities);
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      saveQueue.current = saveQueue.current
        .then(async () => {
          if (epoch === vaultEpoch.current && vaultRef.current === keys)
            await saveEncryptedVault(keys, snapshot);
        })
        .catch(() =>
          setVaultError(
            "Penyimpanan terenkripsi gagal. Pastikan ruang penyimpanan browser tersedia.",
          ),
        );
    }, 180);
  ...
  const applyVaultData = (data = {}) => {
    ...
    setBody(
      data.body || {
        ...createDefaultBody(),
        height: Number(data.profile?.height) || 172,
        weight: Number(data.profile?.weight) || 68,
      },
    );
  ...
  const lockVault = async () => {
    ...
    if (vaultRef.current) {
      await saveEncryptedVault(vaultRef.current, getDemoState());
    }
    vaultEpoch.current += 1;
    vaultRef.current = null;
    setVaultMode("unlock");
  ...
  const filteredActivities = useMemo(
    () =>
      activityType === "All"
        ? activityList
        : activityList.filter((a) => a.type === activityType),
  ...
  const showToast = (title, text = "") =>
    Swal.fire({
      ...
      customClass: { popup: "biosync-toast" },
    });
  const ask = (title, text, confirmText = "Lanjutkan") =>
    showModal({
      title,
      text,
      icon: "question",
  ...
  const exportData = () => {
    ...
    link.download = "biosync-my-data.json";
    link.click();
    URL.revokeObjectURL(link.href);
    showToast("Data berhasil diekspor", "File JSON tersimpan di perangkatmu.");
  };
  const deleteAccount = async () => {
    const result = await ask(
      "Hapus akun demo?",
      "Data demo di browser ini akan dihapus. Tindakan tidak dapat dibatalkan.",
      "Hapus data",
    );
    if (result.isConfirmed) {
      if (saveTimer.current) clearTimeout(saveTimer.current);
  ...
  const editActivity = async (activity) => {
    const result = await showModal({
      ...
      preConfirm: () => {
        ...
        if (!title) {
          Swal.showValidationMessage("Nama aktivitas wajib diisi.");
          return false;
        }
        return {
          title,
          type: document.getElementById("activity-type").value,
          distance: document.getElementById("activity-distance").value.trim(),
          time: document.getElementById("activity-time").value.trim(),
          pace: document.getElementById("activity-pace").value.trim(),
        };
      },
    });
    if (result.isConfirmed) {
      setActivityList((current) =>
        current.map((item) =>
          item.id === activity.id ? { ...item, ...result.value } : item,
        ),
      );
  ...
  const deleteActivity = async (activity) => {
    const result = await ask(
      "Hapus aktivitas ini?",
      "Aktivitas akan dihapus dari data demo lokal.",
      "Hapus aktivitas",
    );
    if (result.isConfirmed) {
      setActivityList((current) =>
  ...
  const createChallenge = async () => {
    const result = await showModal({
      title: "Buat challenge pribadi",
      html: '<div class="onboarding-fields"><label>Nama challenge<input id="challenge-name" maxlength="60" placeholder="Contoh: Jalan kaki 5 hari"></label><label>Target pribadi<input id="challenge-target" maxlength="40" placeholder="Contoh: 30 km minggu ini"></label></div>',
      showCancelButton: true,
      confirmButtonText: "Buat challenge",
      cancelButtonText: "Batal",
      customClass: {
        popup: "biosync-modal",
        confirmButton: "swal-confirm",
      ...
      buttonsStyling: false,
      preConfirm: () => {
        const title = document.getElementById("challenge-name").value.trim();
        const goal = document.getElementById("challenge-target").value.trim();
        if (!title || !goal) {
    ...
    if (result.isConfirmed) {
      ...
      showToast("Challenge dibuat");
    }
  };
  const editChallenge = async (challenge) => {
    const result = await showModal({
      title: "Edit challenge",
      html: `<div class="onboarding-fields"><label>Nama challenge<input id="challenge-name" maxlength="60" value="${escapeHtml(challenge.title)}"></label><label>Target pribadi<input id="challenge-target" maxlength="40" value="${escapeHtml(challenge.goal)}"></label></div>`,
      showCancelButton: true,
      confirmButtonText: "Simpan",
      cancelButtonText: "Batal",
      customClass: {
    ...
    if (result.isConfirmed)
      setUserChallenges((current) =>
        current.map((item) =>
          item.id === challenge.id ? { ...item, ...result.value } : item,
        ),
      );
  };
  const deleteChallenge = async (challenge) => {
    const result = await ask(
      "Hapus challenge pribadi?",
      `${challenge.title} akan dihapus dari data demo.`,
      "Hapus",
    );
    if (result.isConfirmed)
      setUserChallenges((current) =>
  ...
  const beginOnboarding = async () => {
    const goals = await showModal({
      ...
      buttonsStyling: false,
      preConfirm: () =>
        [...document.querySelectorAll(".goal-options input:checked")].map(
          (input) => input.value,
        ),
    });
    ...
    const details = await showModal({
      ...
      html: '<div class="onboarding-fields"><label>Display name<input id="onboard-name" value="Alex Morgan" /></label><label>Age range<select id="onboard-age"><option>18–29</option><option selected>30–39</option><option>40–49</option><option>50+</option></select></label><div><label>Height (cm)<input id="onboard-height" type="number" value="172" /></label><label>Weight (kg)<input id="onboard-weight" type="number" value="68" /></label></div><label>Activity level<select id="onboard-level"><option>Light</option><option selected>Moderate</option><option>Active</option></select></label></div><p class="modal-note">These details are optional and can be changed later.</p>',
      showCancelButton: true,
      confirmButtonText: "Continue",
      cancelButtonText: "Skip for now",
      customClass: {
      ...
      preConfirm: () => ({
        ...
        height: document.getElementById("onboard-height").value,
        weight: document.getElementById("onboard-weight").value,
        level: document.getElementById("onboard-level").value,
      }),
    });
    ...
    const privacy = await showModal({
      ...
      icon: "success",
      showCancelButton: true,
      confirmButtonText: "Agree & open dashboard",
      cancelButtonText: "Skip for now",
      customClass: {
        popup: "biosync-modal",
        confirmButton: "swal-confirm",
    ...
    if (privacy.isConfirmed || privacy.isDismissed) {
      if (detailsValue.name)
        setProfile((old) => ({
          ...old,
          name: detailsValue.name,
          goals: selectedGoals,
          age: detailsValue.age,
          height: detailsValue.height,
          weight: detailsValue.weight,
          activityLevel: detailsValue.level,
        }));
      setBody((old) => ({
        ...old,
  ...
  const joinChallenge = (title) => {
    ...
    showToast(
      joined.includes(title) ? "Keluar dari challenge" : "Kamu bergabung!",
      title,
    );
  };
  ...
  if (vaultStatus === "unavailable")
    return (
      <div className="vault-checking">
        Browser ini tidak menyediakan penyimpanan lokal yang diperlukan.
      </div>
    );
    ...
    <div className="app-shell">
      <aside className={`sidebar ${drawer ? "sidebar-open" : ""}`}>
        <div className="brand-row">
          ...
          <span>
            bio<span className="brand-light">sync</span>
          </span>
          <button
        ...
        <nav className="side-nav">
          {navItems.map(({ label, icon: Icon }) => (
            ...
            >
              ...
              {label === "Challenges" && <span className="nav-dot" />}
            </button>
          ))}
        </nav>
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="top-actions">
            <button
              ...
              onClick={() =>
                showModal({
                  title: "Pusat bantuan BioSync",
                  text: "Untuk MVP demo, fitur ini belum terhubung ke layanan dukungan.",
                  icon: "info",
            ...
            <button
              ...
              onClick={() => showToast("Kamu sudah up to date")}
            >
              <Bell size={18} />
              <i />
            </button>
            <div className="top-divider" />
            <button className="top-user" onClick={() => changePage("Profile")}>
        ...
        <div className="page-content">
          {vaultError && (
            <div className="vault-save-error" role="alert">
              {vaultError}
            </div>
          )}
          {page === "Home" && (
        ...
        <footer className="footer">
          ...
          <span>
            Made for a healthier you <span className="footer-heart">♥</span>
          </span>
          <span>
              ...
              <div className="preview-chart">
                ...
                <svg viewBox="0 0 360 88" preserveAspectRatio="none">
                  <path
                    ...
                    d="M0 71 C26 70 26 45 55 53 S87 64 113 38 S148 48 170 31 S205 41 225 23 S259 48 286 29 S329 27 360 10 V88 H0Z"
                  />
                  <path
                    className="preview-line"
                    d="M0 71 C26 70 26 45 55 53 S87 64 113 38 S148 48 170 31 S205 41 225 23 S259 48 286 29 S329 27 360 10"
                  />
                </svg>
        ...
        <section className="landing-feature-row" id="features">
          ...
          <div className="landing-feature-card">
            ...
            <div className="landing-feature-copy">
              <b>{items[feature].title}</b>
              <p>{items[feature].copy}</p>
            </div>
            <div className="landing-feature-controls">
              <button
                onClick={() =>
      ...
      <div className="welcome-row">
        <div>
          <div className="eyebrow">
            <span className="live-dot" /> {getTodayLabel()}
          </div>
          <h1>
            Good morning, {firstName}
            <span className="wave">✦</span>
          </h1>
          <p>Hari baru, kesempatan baru untuk bergerak lebih baik.</p>
        </div>
        <button className="date-button">
          <CalendarDays size={16} /> Hari ini <ChevronDown size={14} />
        </button>
      </div>
      {loading ? (
        <DashboardSkeleton />
        ...
        <>
          <section className="hero-grid">
            <div className="score-card">
              <div className="score-orb orb-one" />
              <div className="score-orb orb-two" />
              <div className="score-top">
                <div>
                  <div className="card-kicker">
                    YOUR DAILY SCORE <span className="info-dot">i</span>
                  </div>
                ...
                <div className="score-badge">
                  <TrendingUp size={14} /> +8 pts
                </div>
              </div>
              <p className="score-copy">
                Kamu lebih aktif dari <b>78% pengguna</b> hari ini.
              </p>
            ...
            <div className="streak-card">
              ...
              <div className="week-dots">
                {["M", "T", "W", "T", "F", "S", "S"].map((d, i) => (
                  <div className={i < 5 ? "done" : ""} key={`${d}${i}`}>
                    <i>{i < 5 ? <Check size={11} /> : ""}</i>
                    <span>{d}</span>
                  </div>
                ))}
              </div>
              <div className="streak-note">
                <span className="streak-spark">✦</span> Keep it up! 3 more days
                to your next badge.
              </div>
            </div>
          </section>
          ...
          <section className="content-grid">
            <div className="panel activity-chart-panel">
              <div className="panel-heading">
                <div>
                  <h2>Activity overview</h2>
                  <p>Keep your movement in rhythm.</p>
                </div>
                <select
              ...
              <div className="chart-legend">
                <span>
                  <i className="legend-teal" /> Steps
                </span>
                <b>
                  8,420 <small>steps</small>
                </b>
              </div>
              <div className="chart-wrap">
                <Suspense fallback={<div className="skeleton chart-loading" />}>
              ...
              <div className="chart-foot">
                <span>
                  ...
                  <b>+18.4%</b> <span>vs last week</span>
                </span>
                <button
            ...
            <div className="panel snapshot-panel">
              <div className="panel-heading">
                <div>
                  ...
                  <p>Your body, at a glance.</p>
                </div>
                <button
                  className="dots-button"
                  onClick={() => changePage("Health")}
              ...
              <div className="snapshot-item">
                <div className="snap-icon sleep-icon">
                  <Moon size={17} />
                </div>
                ...
                <div className="sleep-bars">
                  {[
                    10, 14, 12, 19, 14, 22, 16, 24, 16, 20, 15, 12, 17, 13, 21,
                    15, 12, 19, 11, 15,
                  ].map((h, i) => (
              ...
              <div className="snapshot-item">
                <div className="snap-icon recovery-icon">
                  <Zap size={17} />
                </div>
                <div className="snap-main">
                  <span>Recovery</span>
                ...
                <span className="status-pill great">Optimal</span>
                <div className="recovery-meter">
                  <span />
                </div>
              </div>
              <div className="snapshot-item">
                <div className="snap-icon oxygen-icon">
                  <Waves size={17} />
                </div>
                <div className="snap-main">
                ...
                <span className="status-pill good">Normal</span>
                <svg
                  className="mini-line"
                  viewBox="0 0 90 24"
                  preserveAspectRatio="none"
                >
                  <path d="M1 15 C8 12 8 18 14 14 S22 11 28 14 S36 18 42 11 S50 17 56 13 S64 12 70 15 S80 7 89 11" />
                </svg>
              </div>
              <button
                ...
                onClick={() => changePage("Health")}
              >
                See all health metrics <ArrowRight size={14} />
              </button>
            </div>
          </section>
          <section className="lower-grid">
            ...
            <div className={`challenge-feature feature-${featured.tone}`}>
              <div className="challenge-feature-top">
                <span className="feature-tag">
                  <Sparkles size={12} /> WEEKLY CHALLENGE
                </span>
                <div className="carousel-controls">
                  <button
                    onClick={() =>
                      setSlide(
                        (slide + challenges.length - 1) % challenges.length,
                      )
                    }
                    aria-label="Challenge sebelumnya"
                  >
                    <ChevronLeft size={15} />
                  </button>
                  <button
                    onClick={() => setSlide((slide + 1) % challenges.length)}
                    aria-label="Challenge berikutnya"
                  >
                    <ChevronRight size={15} />
                  </button>
                </div>
              ...
              <div className="feature-icon">
                <featured.icon size={21} />
              </div>
              <h2>{featured.title}</h2>
              <p>{featured.sub}</p>
              <div className="feature-progress-line">
              ...
              <button
                ...
                onClick={() => joinChallenge(featured.title)}
              >
                {joined.includes(featured.title)
                  ? "Joined ✓"
                  : "Join challenge"}{" "}
                <ArrowRight size={15} />
              </button>
          ...
          <section className="bottom-promo">
            <div className="promo-shield">
              <ShieldCheck size={21} />
            </div>
            <div>
              <b>Your health data is yours.</b>
              <p>Review what you share, and stay in control at every step.</p>
            </div>
            <button onClick={() => changePage("Privacy")}>
              Privacy center <ArrowRight size={14} />
            </button>
  ...
  return (
    <div className="metric-card">
      <div className="metric-top">
        <div className={`metric-icon ${tint}`}>
          <Icon size={17} />
        </div>
        <span className={`metric-change ${down ? "down" : ""}`}>
      ...
      <div className="metric-progress">
        <span className={tint} style={{ width: `${progress}%` }} />
      </div>
      <div className="metric-foot">{goal}</div>
    </div>
  );
}
function ActivityRow({ item, onEdit, onDelete }) {
  ...
  return (
    <div className="activity-row">
      <div className={`activity-symbol ${item.tone}`}>
        <Icon size={17} />
      </div>
      ...
      <button
        ...
        onClick={async () => {
          if (!onEdit || !onDelete)
            return showModal({
              title: item.title,
              html: `<div class="activity-detail"><span>${escapeHtml(item.distance)} <small>Distance</small></span><span>${escapeHtml(item.time)} <small>Duration</small></span><span>${escapeHtml(item.pace)} <small>Average pace</small></span></div>`,
          ...
          const result = await showModal({
            title: item.title,
            text: `${item.distance} · ${item.time} · ${item.pace}`,
            icon: "info",
            showCancelButton: true,
            showDenyButton: true,
          ...
          if (result.isConfirmed) onEdit(item);
          if (result.isDenied) onDelete(item);
        }}
      >
        <MoreHorizontal size={18} />
      </button>
...
function DashboardSkeleton() {
  return (
    <div className="skeleton-dashboard" aria-label="Memuat dashboard">
      <div className="skeleton skeleton-hero" />
      <div className="skeleton-row">
        {[1, 2, 3].map((n) => (
          <div className="skeleton skeleton-metric" key={n} />
        ))}
      </div>
      <div className="skeleton-row">
        <div className="skeleton skeleton-chart" />
        <div className="skeleton skeleton-snapshot" />
      </div>
      <div className="skeleton-row">
        <div className="skeleton skeleton-recent" />
        <div className="skeleton skeleton-challenge" />
      </div>
    </div>
  );
}
...
function ActivityPage({
  ...
  return (
    <>
      <PageHeader
        eyebrow="MOVE AT YOUR PACE"
        title="Your activity"
        subtitle="Small steps. Stronger you."
        action={
          <button
            className={`primary-button ${tracking ? "recording" : ""}`}
            onClick={startTracking}
          >
            {tracking ? (
              <>
                <span className="record-dot" /> {duration} · Finish
              </>
            ) : (
              <>
                <Plus size={17} /> Start activity
              </>
            )}
          </button>
        }
      ...
      <div className="activity-summary">
        <div>
          ...
          <b>
            {summary.distance} <small>km</small>
          </b>
          <small className="summary-up">Total recorded</small>
        </div>
        <div>
          <span>Active time</span>
          <b>{summary.activeTime}</b>
          <small>across {summary.count} activities</small>
        </div>
        <div>
          <span>Avg. pace</span>
          <b>
            {summary.pace} <small>/km</small>
          </b>
          <small>Run and walk average</small>
        </div>
      </div>
      ...
      <div className="panel history-panel">
        {activities.length ? (
          ...
          <EmptyState
            ...
            copy="Mulai bergerak dan aktivitasmu akan muncul di sini."
          />
        )}
      </div>
...
function HealthPage({ loading, range, setRange, exportData, body, setBody }) {
  ...
  return (
    <>
      <PageHeader
        eyebrow="KNOW YOUR BODY"
        title="Health overview"
        subtitle="Patterns, not diagnoses. See how you’re doing over time."
        action={
          <select
            className="range-select"
            value={range}
      ...
      <div className="health-tabs">
        {["Overview", "Sleep", "Heart", "Body", "Recovery"].map((item) => (
          <button
      ...
      {tab === "Body" ? (
        <BodyGoals body={body} setBody={setBody} />
      ) : (
        <>
            ...
            {[
              {
                name: "Sleep duration",
                value: "7h 42m",
                label: "A little above your weekly average",
                icon: Moon,
                tone: "purple",
                status: "GOOD",
              },
              {
                ...
                value: "58 bpm",
                label: "Stable over the past 7 days",
                icon: Heart,
                tone: "pink",
                status: "NORMAL",
              },
              {
                name: "Recovery score",
                value: "86",
                label: "Ready for moderate activity today",
                icon: Zap,
                tone: "teal",
                status: "OPTIMAL",
              },
            ].map(({ name, value, label, icon: Icon, tone, status }) => (
              <div className="health-metric panel" key={name}>
                ...
                <div className="health-sparkline">
                  <Suspense
                    fallback={<div className="skeleton chart-loading" />}
                  >
          ...
          <div className="panel health-trend">
            <div className="panel-heading">
              <div>
                <h2>{tab} trends</h2>
                <p>Keep an eye on patterns over time.</p>
              </div>
            ...
            <div className="health-disclaimer">
              <ShieldCheck size={15} /> BioSync memberikan insight informatif,
              bukan diagnosis atau pengganti konsultasi tenaga medis.
            </div>
          </div>
          <div className="health-export panel">
            <div className="export-icon">
              <LockKeyhole size={20} />
            </div>
            <div>
              <b>Data health, under your control.</b>
              <p>Export a copy or review your data permissions at any time.</p>
            </div>
            <button className="secondary-button" onClick={exportData}>
              Export health data <ArrowRight size={14} />
            </button>
          </div>
        </>
      )}
...
function ChallengesPage({ joined, joinChallenge, createChallenge }) {
  return (
    <>
      <PageHeader
        eyebrow="MOVE TOGETHER"
        title="Challenges"
        subtitle="Make consistency feel like a team sport."
        action={
          <button className="secondary-button" onClick={createChallenge}>
      ...
      <div className="challenge-banner">
        <div className="banner-content">
          <span className="feature-tag">
            <Sparkles size={12} /> COMMUNITY PICK
          </span>
          <h2>Better together.</h2>
          <p>
            Join a community challenge and turn your daily movement into a
            shared win.
          </p>
          <button
            onClick={() =>
              document
                .getElementById("challenge-list")
        ...
        <div className="banner-orbit">
          ...
          <span className="orbit-person p-one">A</span>
          <span className="orbit-person p-two">M</span>
          <span className="orbit-person p-three">J</span>
        </div>
      </div>
      <div className="challenge-toolbar" id="challenge-list">
        <div>
          <h2>For you</h2>
          <p>Pick a goal that fits your week.</p>
        </div>
      ...
      <div className="challenge-grid">
        {challenges.map((item, i) => {
          ...
          const isJoined = joined.includes(item.title);
          return (
            <div
              className={`challenge-card panel challenge-${item.tone}`}
              key={item.title}
            >
              <div className="challenge-card-head">
                <div className={`challenge-card-icon ${item.tone}`}>
                  <Icon size={20} />
                </div>
                <button
                  className="dots-button"
                  aria-label="Opsi challenge"
                  onClick={() =>
                    showModal({
                      title: item.title,
                      text: `${item.people} · ${item.sub}`,
                      icon: "info",
                      confirmButtonText: "Tutup",
                      customClass: {
                        popup: "biosync-modal",
              ...
              <span className="challenge-category">
                {i === 0 ? "DISTANCE" : i === 1 ? "WELLNESS" : "DAILY GOAL"}
              </span>
              <h3>{item.title}</h3>
              <p>{item.sub}</p>
              ...
              <div className="challenge-card-bottom">
                ...
                <button
                  ...
                  onClick={() => joinChallenge(item.title)}
                >
      ...
      <div className="badge-section">
        <div className="section-title-row">
          <div>
            <h2>Your badges</h2>
            <p>Little wins worth celebrating.</p>
          </div>
          <span className="badge-count">3 earned</span>
        </div>
          ...
          {[
            {
              icon: Footprints,
              title: "First Move",
              sub: "Your first activity",
            },
            { icon: Flame, title: "7-Day Streak", sub: "Moved for 7 days" },
            { icon: Target, title: "Goal Getter", sub: "Reached a daily goal" },
            { icon: Heart, title: "Early Bird", sub: "Locked and loaded" },
          ].map(({ icon: Icon, title, sub }, i) => (
            <div
              className={`badge-item ${i === 3 ? "locked" : ""}`}
              key={title}
            >
...
function MyChallenges({ challenges, editChallenge, deleteChallenge }) {
  ...
  return (
    <section className="panel my-challenges">
      <div className="section-title-row">
        <div>
          <h2>Challenge pribadi</h2>
          <p>Data demo yang kamu buat.</p>
        </div>
      </div>
      <div className="my-challenge-list">
        {challenges.map((item) => (
          <article className="my-challenge-row" key={item.id}>
            <div className="challenge-card-icon teal">
              <Target size={18} />
            </div>
            <div className="my-challenge-copy">
              <b>{item.title}</b>
              <span>Target: {item.goal}</span>
            </div>
            <button className="text-button" onClick={() => editChallenge(item)}>
              Edit
            </button>
            <button
              className="delete-button"
              onClick={() => deleteChallenge(item)}
            >
              Hapus
            </button>
          </article>
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Transparency and control, built into every step."
        action={
          <div className="privacy-safe">
            <ShieldCheck size={15} /> Privacy protected
          </div>
        }
      />
      <div className="privacy-hero">
        ...
        <div>
          <span className="eyebrow">PRIVACY OVERVIEW</span>
          <h2>You’re in control, always.</h2>
          <p>
            We only use the data you choose to share. Your health details stay
            private and are never sold.
          </p>
        </div>
      ...
      <div className="privacy-grid">
        <div className="panel privacy-card">
          <div className="panel-heading">
            <div>
              <h2>Your data</h2>
              <p>What’s currently stored in this demo.</p>
            </div>
          ...
          <div className="data-row">
            ...
            <b>{connections.length} selected for demo</b>
          </div>
          <div className="data-row">
            <span>Wallet / blockchain</span>
            <span className="optional-label">Optional · not connected</span>
          </div>
          <button className="secondary-button full-width" onClick={exportData}>
            Download my data <ArrowRight size={14} />
          </button>
        </div>
        <div className="panel privacy-card">
          <div className="panel-heading">
            <div>
              ...
              <p>Review the categories selected for each demo source.</p>
            </div>
          ...
          {connections.length ? (
            <div className="permission-apps">
              {connections.map((name) => (
                <div className="permission-app" key={name}>
                  <div className="permission-avatar">{name[0]}</div>
                  <div>
                    <b>{name}</b>
                    <span>
                      {(devicePermissions[name] || [])
                        .map(
                          (key) =>
                            ({
                              activity: "Langkah & aktivitas",
                              heart: "Detak jantung",
                              sleep: "Tidur & recovery",
                            })[key],
                        )
                        .filter(Boolean)
                        .join(", ") || "Tidak ada kategori diizinkan"}{" "}
                      · Sampai dicabut
                    </span>
                  </div>
                  <button
                    className="toggle-switch on"
                    aria-label={`Cabut izin ${name}`}
                    onClick={() => {
                      setConnections((current) =>
            ...
            <div className="permission-empty">
              <LockKeyhole size={20} />
              <span>Belum ada aplikasi yang memiliki akses.</span>
            </div>
          )}
          <div className="consent-row">
            <div>
              <b>Product improvement</b>
              <span>Share anonymous usage insights</span>
            </div>
            <button
              className={`toggle-switch ${consent ? "on" : ""}`}
              aria-label="Ubah izin penggunaan data"
              onClick={() => {
                setConsent(!consent);
                showToast(consent ? "Consent dicabut" : "Consent diperbarui");
              }}
            />
          </div>
        </div>
      ...
      <div className="privacy-action-row">
        <div className="privacy-action">
          <div className="action-symbol export">
            <ArrowDownRight size={18} />
          </div>
          <div>
            <b>Download your data</b>
            <span>Get a portable copy in JSON format.</span>
          </div>
          <button className="secondary-button" onClick={exportData}>
            Export data <ArrowRight size={14} />
          </button>
        </div>
        <div className="privacy-action delete-action">
          <div className="action-symbol delete">
            <X size={18} />
          </div>
          <div>
            <b>Delete your account</b>
            <span>Permanently remove your local demo data.</span>
          </div>
      ...
      <div className="blockchain-note">
        ...
        <span>
          <b>Web3 ownership · Beta</b>
          <small>
            Wallet connection is optional. Health details are never written to a
            public blockchain.
          </small>
        </span>
        ...
        <button
          onClick={() =>
            showToast(
              "Web3 beta",
              "Wallet connect is planned for a later phase.",
            )
          }
        >
...
async function editHealthGoals(profile, setProfile) {
  ...
  const result = await showModal({
    title: "Health goals",
    html: `<div class="consent-options goal-options">${options.map((goal) => `<label><input type="checkbox" value="${goal}" ${profile.goals?.includes(goal) ? "checked" : ""}> ${goal}</label>`).join("")}</div>`,
    showCancelButton: true,
    confirmButtonText: "Simpan",
    cancelButtonText: "Batal",
    customClass: {
  ...
  if (result.isConfirmed)
    setProfile((current) => ({ ...current, goals: result.value }));
}
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="A little about you, on your terms."
      />
      <div className="profile-layout">
        <div className="panel profile-card">
          <div className="profile-cover">
            <div className="profile-avatar-large">{initials(profile.name)}</div>
          </div>
          <div className="profile-summary">
            <h2>{profile.name}</h2>
            <span>Personal account · Member since Sep 2026</span>
        ...
        <div className="panel settings-card">
          ...
          <label className="form-label">
            ...
            <small>Email changes aren’t available in this demo.</small>
          </label>
          <div className="settings-divider" />
          <div className="settings-row">
            <div>
              <b>Health goals</b>
              <span>{profile.goals?.join(" · ") || "Belum ada target"}</span>
            </div>
            <button
              className="text-button"
              onClick={() => editHealthGoals(profile, setProfile)}
            >
          ...
          <div className="settings-row">
            <div>
              ...
              <span>Manage health data sources</span>
            </div>
            <button
              className="text-button"
              onClick={() => changePage("Devices")}
          ...
          <div className="settings-row">
            <div>
              ...
              <span>Review what you’re sharing</span>
            </div>
            <button
              className="text-button"
              onClick={() => changePage("Privacy")}
          ...
          <div className="settings-actions">
            <button className="secondary-button" onClick={exportData}>
              Export my data <ArrowDownRight size={14} />
            </button>
            <button
              className="primary-button"
              onClick={() => {
                setProfile({ ...profile, name: name || profile.name });
                setSaved(true);
      ...
      <div className="logout-row">
        <span>
          <LogOut size={16} /> Want to remove your data?
        </span>
        <button className="delete-button" onClick={deleteAccount}>
          Delete account
        </button>
